...
## 2-5. 그리드 배치 — 비율이 섞인 "justified" 레이아웃 (3개씩 한 줄)

전부 같은 크기였던 균일 그리드 대신, 이제는 프로젝트마다 다른 가로세로 비율(`ratio`)을
가질 수 있는 레이아웃입니다. `script.js`의 `PROJECTS` 배열 순서를 그대로 따라가며,
`GRID_ROW_SIZE`(기본값 3)개씩 끊어서 한 줄이 됩니다:

```
Desert(16:9)      Gate(16:9)      Pillar(9:16, 세로)
Trebuchet(약간 가로형)  Alley(정사각)   Computer(정사각)
Sci-fi Container(정사각)  House(16:9)  Zbrush(정사각)
```

같은 줄 안에서는 `ratio`가 큰(가로로 넓은) 타일일수록 더 넓게, 작은(세로로 긴) 타일일수록
더 좁게 자동으로 너비가 배분되고, 그 대신 같은 줄의 **높이는 항상 통일**됩니다. 그리고 모든
줄의 전체 너비는 항상 콘텐츠 영역 폭과 정확히 같기 때문에(= 줄마다 좌우 끝이 항상 일치),
비율이 서로 다른 타일이 섞여도 전체 그리드 외곽선은 반듯한 직사각형을 유지합니다.

- **비율을 바꾸고 싶으면**: `PROJECTS` 배열의 각 항목에 있는 `ratio` 값(가로/세로)만 고치면
  됩니다. `1`은 정사각형, `16 / 9`는 가로로 넓은 형태, `9 / 16`은 세로로 긴 형태입니다.
  정확한 숫자를 지킬 필요는 없고, 그 줄의 전체 균형(한 타일만 유난히 좁아지지 않도록)을
  보면서 조금씩 조절해도 됩니다.
- **배치 순서를 바꾸고 싶으면**: `PROJECTS` 배열 안 항목의 순서만 바꾸면 됩니다
  (`GRID_ROW_SIZE`개씩, 기본 3개씩 끊어서 한 줄이 됩니다).
- **한 줄에 몇 개씩 넣을지 바꾸고 싶으면**: `script.js`의 `GRID_ROW_SIZE` 값을 고치면 됩니다.
- 화면이 좁아지면(태블릿/모바일) 세로형 타일이 지나치게 좁아지는 걸 막기 위해, 900px 이하부터는
  비율 대신 한 줄에 2개씩(4:3), 520px 이하에서는 1개씩(가로 꽉 채움)으로 자동 전환됩니다.
...
- **그리드 타일 비율/크기**: `script.js`의 `PROJECTS` 배열 각 항목의 `ratio` 값 (레이아웃 구조 자체를
  바꾸고 싶다면 `style.css`의 `.grid-row`, `.tile` 블록)
